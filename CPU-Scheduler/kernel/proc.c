...
userinit(void)
{
  ...
  p->cwd = namei("/");
  p->state = RUNNABLE;
  //since p->state is RUNNABLE add to the first queue.
  struct proc *temp;
  int index=0;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    intr_on();
    //it means that there is something runnable in first queue which is RUNNABLE
    //we don't check the tick in the first queue because the quanta is 1 ticks.
    //which means that after one switch it pass to the second queue.
    if(qtable[NPROC]!=-1){
      p = &proc[qtable[NPROC]];
      acquire(&p->lock);
      p->state = RUNNING;
    ...
    //there is something runnable in second queue
    ...
    //there is something runnable in third queue
