...
int executeMove(char** dataTotal, int* dataSize)
{
	...
	while (data[j] != '\n')
	{
		...
		if (data[j - 1] == '.' || data[j - 1] == 'r')
		{
			check = StockToWaste(getTurns(), data[j - 1], dataTotal);
			countMove++;
			if (check == 0)
			{
				printf("Move %d is Illegel : %c -> %c\n", j, data[j - 1], data[j + 1]);
	...
	if (noError == 0)
	{
		printf("Processed %c moves , all valid\n", countMove);
	}
	return result;
}
...
int moveTableau(char** dataTotal, char a, char b, int* dataSize)
{
	...
	for (int i = times; i > 0; i--)
	{
		...
		{
			//1 succeed 0 error
			temp = temp + saveFoundation(dataTotal, dataSize, num, shape) == 1;
		}
		else
		{
			//1 succeed 0 error
...
int main() {
	...
	//WasteToFoundationOrTableau('w', '4', data, dataSize);
	////moveTableau(data, 4, 7, dataSize);
